...
fn mac_build(target: BuildTarget, release: bool) -> Result<()> {
    ...
    for (arch, triple) in mac_triples(target) {
        println!("\n── Target: {triple} ──");
        match kind {
            ProjectKind::Tauri => build_tauri_mac(&project_dir, triple, mode_label, release, use_mise)?,
            ProjectKind::Cargo => build_cargo_mac(&project_dir, &artifacts_dir, arch, triple, mode_label, release, use_mise)?,
        }
    }
    Ok(())
}
...
fn ensure_mise_installed(project_dir: &std::path::Path) -> Result<bool> {
    ...
    match (has_mise_toml, mise_on_path) {
        (true, true) => {
            ...
            if !status.success() { bail!("mise install exited with {}", status); }
            Ok(true)
        }
        (true, false) => {
